...
<thead>
  ...
  <th>Producto</th>
  <th style="width: 30px;">Precio x Mayor</th>
...
<tr >
  ...
  <td><?php echo $product->name; ?></td>
  <td><b>$ <?php echo number_format($product->price_mayor,2,".",","); ?></b></td>

  <td><b>$ <?php  $pt = $product->price_mayor*$p["q"]; $total +=$pt; echo number_format($pt,2,".",","); ?></b></td>
...
<script>
  $("#clearcart-<?php echo $product->id; ?>").click(function(){
    $.get("index.php?view=clearcart","product_id=<?php echo $product->id; ?>",function(data){
        $.get("./?action=cartofsellm",null,function(data2){
          $("#cartofsellm").html(data2);
        });
    });
  });
</script>
...
<form method="post" class="form-horizontal" id="processsellm" enctype="multipart/form-data">
<h2>Resumen</h2>
...
<div class="col-md-12">
    ...
    <div class="col-lg-12">
      <input  type="hidden" name="tipo_m" class="form-control"  placeholder="tipo_m" value="mayor">
    </div>
  </div>
  </div>
<div class="row">
...
<script>
	$("#processsellm").submit(function(e){
    ...
    if(p!=4){
		if(money<(<?php echo $total;?>-discount)){
			...
			go = confirm("Cambio: $"+(money-(<?php echo $total;?>-((discount * <?php echo $total;?>)/100) ) ) );
			if(go){
      e.preventDefault();
        $.post("./index.php?action=processsellm",$("#processsellm").serialize(),function(data){
          $.get("./?action=cartofsellm",null,function(data2){
            $("#cartofsellm").html(data);
