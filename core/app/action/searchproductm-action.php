...
if($product->kind==1){
  ...
  ?>
		...
		<td><b>$<?php echo $product->price_mayor; ?></td>
		<td>
			<?php 
			if($product->kind==1){
			echo $q; 
		...
		<form method="post" action="index.php?view=addtocartm" id="addtocartm<?php echo $product->id;?>">
...
<!--<script>
		$("#addtocartm<?php echo $product->id;?>").on("submit",function(e){
		e.preventDefault();
			$.post("./?view=addtocartm",$("#addtocartm<?php echo $product->id;?>").serialize(),function(data){
